...
class TestExector(threading.Thread):
    def __init__(self,queue,date,niDevName,rs,objectsLists,configFile,product="10FA"):
    ...
    def compute(self,test,data,temperature):
        ...
        if test=="lp":
            lpConfig=self.getConfig('lp',temperature)
            result=gjb2426.lp(data,int(lpConfig["stableFactor"]))
        elif test=="lp_cfx":
            result=gjb2426.lp_cfx(data)
        elif test=="bdys":
            result=gjb2426.bdys(data,speeds)
        elif test=="bdys_cfx":
            result=gjb2426.bdys_cfx(data,speeds)
        else:
        ...
        if result and len(result)==len(self.objectLists):
            ...
            for i in range(len(bhIndex)):
                ...
                with open(resultFile,'a') as f:
                    f.write("\n".join(list(map(str, temp[dataKey]))))
                if test=="lp":
                    db.insert_lp(self.product,self.testDate,temperature,bh,temp['lp'],temp['lp_wdx'],resultFile)
                    with open(resultFile,'a') as f:
                        f.write('\n\n')
                        f.write('测试日期：' +str(self.testDate)+'\n')
                        f.write('零位: '+ str(temp['lp']) +'\n' )
                        f.write('零位稳定性: '+ str(temp['lp_wdx']) +'\n' )
                elif test=="lp_cfx":
                    db.insert_lp_cfx(self.product,self.testDate,temperature,bh,temp['lp_cfx'],resultFile)
                elif test=="bdys":
                    db.insert_bdys(self.product,self.testDate,temperature,bh,temp['bdys'],temp['bdys_xxd'],resultFile)

                elif test=="bdys_cfx":
                    db.insert_bdys_cfx(self.product,self.testDate,temperature,bh,temp['bdys'],temp['bdys_xxd'],resultFile)
        else:
            del db
    ...
    def bdysRepeat(self,temperature):
        ...
        repeatNums=int(bdysRepeatConfig["repetation"])
        res=[]
        for i in range(repeatNums):
